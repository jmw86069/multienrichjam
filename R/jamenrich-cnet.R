...
#' @param percent_spacing `numeric` within node spacing as percentage of
#'    the max x/y layout ranges. This option is preferred to `expand`
#'    when trying to keep nodeset spacing consistent for the overall layout.
#'    As a general rule, use `summarize_node_spacing(g)$nearest_within`
#'    with column 'Median' to determine node spacing for each nodeset,
#'    then adjust each nodeset to relatively consistent median spacing.
...
#'    Otherwise `get_cnet_nodesets()` is called, which for
#'    large networks may introduce a small time lag.
#' @param verbose `logical` indicating whether to print verbose output.
#' @param ... additional arguments are ignored.
#'
#' @export
