...
The object returned `Mpf` is a `MemPlotFolio` object containing the
...
* `CnetExemplar(Mpf, num=1)`
* `CnetCluster(Mpf, cluster="B")`
...
# extract the cnet
cnet <- CnetCollapsed(Mpf4, do_plot=FALSE, type="set");
...
`EnrichmentHeatmap(Mpf)` with the 'MemPlotFolio' object as input.
Customization should be done via `mem_plot_folio()`
to ensure that all other plots use the same parameters.

The specific options for a enrichment heatmap are described in
...
The recommended way to create an enrichment heatmap is to use
`GenePathHeatmap(Mpf)` with the 'MemPlotFolio' object as input.
Customization should be done via `mem_plot_folio()`
to ensure that all other plots use the same parameters.
...
The corresponding custom function is shown below:
